...
public class BaseRoad implements Serializable {
	 ...
	 * @param geometry
	 *            Road's geometry from source to target as {@link Polyline}
	 *            object.
	 */
	 ...
	 * <p>
	 * <b>Note:</b> A routable road map requires splitting of roads into
	 * segments to build a road topology (graph). Since OpenStreetMap roads span
	 * often roads over multiple intersections, they must be split into multiple
	 * road segments. Hence, it is a one-to-many relationship.
	 *
	...
	/**
	 * Gets road's priority factor, i.e. an additional cost factor for routing,
	 * and must be greater or equal to one. Higher priority factor means higher
	 * costs.
	 *
	 ...
	 * road's source to its target.
	 *
	 * @return Road's geometry as a {@link ByteBuffer} in WKB format from the
	 *         road's source to its target.
	 */
	public byte[] wkb() {
		return geometry;
