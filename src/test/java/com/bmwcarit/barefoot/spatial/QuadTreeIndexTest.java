...
public class QuadTreeIndexTest {
    ...
    public void testIndexNearest() {
...

        {
            ...
            Set<Tuple<Long, Double>> points = index.nearest(c);

            assertEquals(1, points.size());
            assertEquals(Long.valueOf(nearest), points.iterator().next().one());
        }
        {
    ...
    public void testIndexRadius() {
...

        {
            ...
            for (long i = 0; i < lines.size(); ++i) {
                double f = spatial.intercept(lines.get((int) i), c);
                Point p = spatial.interpolate(lines.get((int) i), f);
                double d = spatial.distance(c, p);
            ...
            Set<Tuple<Long, Double>> points = index.radius(c, r);
    ...
    public void testIndexKNearest() {
        final String[] wkts = {"LINESTRING (11.4235408 48.1010922, 11.4235077 48.101068, 11.4231626 48.1008749)",
        ...
        {
            final List<Triple<Long, Polyline, Double>> lines = new ArrayList<>();
            ...
            for (String wkt : wkts) {
                long id = i++;
                Polyline line = (Polyline) GeometryEngine.geometryFromWkt(wkt, WktImportFlags.wktImportDefaults,
            ...
            Collections.sort(lines, new Comparator<Triple<Long, Polyline, Double>>() {
                @Override
                public int compare(Triple<Long, Polyline, Double> left, Triple<Long, Polyline, Double> right) {
            ...
            for (long j = 0; j < k; ++j) {
                Tuple<Long, Polyline> e = lines.get((int) j);
                neighbors.add(e.one());
            }

            Set<Tuple<Long, Double>> points = index.knearest(c, k);
        ...
        }
        {
            ...
            for (int j = 0; j < k; ++j) {
                Triple<Long, Polyline, Double> e = lines.get(j);
                neighbors.add(e.one());
            }
        ...
        }
        {
            ...
            for (int j = 0; j < k; ++j) {
                Tuple<Long, Polyline> e = lines.get(j);
                neighbors.add(e.one());
            }
